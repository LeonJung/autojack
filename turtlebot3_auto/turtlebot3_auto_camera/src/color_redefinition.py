...
class ColorRedefinition():
    def __init__(self):
        ...
        if self.selecting_pub_image == "compressed":
            # self._pub1 = rospy.Publisher('/image_redefined_compressed', CompressedImage, queue_size = 1)
            # self._pub4 = rospy.Publisher('/image_redefined_compressed3', CompressedImage, queue_size= 1)
            self._pub5 = rospy.Publisher('/image_redefined_compressed2', CompressedImage, queue_size= 1)
        elif self.selecting_pub_image == "raw":
            # self._pub2 = rospy.Publisher('/image_redefined', Image, queue_size = 1)
            # self._pub3 = rospy.Publisher('/image_redefined3', Image, queue_size=1)
            self._pub6 = rospy.Publisher('/image_redefined2', Image, queue_size=1)
    ...
    def callback(self, image_msg):
        ...
        # img_output = cv2.LUT(cv_image, table)
        ...
        # new_img_lab = cv2.merge((cl1, a, b))

        # img_output2 = cv2.cvtColor(new_img_lab, cv2.COLOR_LAB2BGR)
...
#             maxGray--;
            ...
            # hist = cv2.calcHist([cv_image], [0], None, [hist_size], [0, hist_size])
...
# }
        ...
        # cv2.imshow('Color input image', cv_image)
        ...
        if self.showing_images == "on":
            ...
            # cv2.namedWindow('image_output', cv2.WINDOW_AUTOSIZE)
            # cv2.moveWindow('image_output', 400, 250)
            # cv2.imshow('image_output', img_output), cv2.waitKey(1)
            # cv2.namedWindow('image_output2', cv2.WINDOW_AUTOSIZE)
            # cv2.moveWindow('image_output2', 800, 250)
            # cv2.imshow('image_output2', img_output2), cv2.waitKey(1)
        ...
        if self.selecting_pub_image == "compressed":
            ...
            # msg_final_img.data = np.array(cv2.imencode('.jpg', img_output)[1]).tostring()
            # self._pub1.publish(msg_final_img)

            # msg_final_img2 = CompressedImage()
            # msg_final_img2.header.stamp = rospy.Time.now()
            # msg_final_img2.format = "jpeg"
            # msg_final_img2.data = np.array(cv2.imencode('.jpg', img_output2)[1]).tostring()
            # self._pub4.publish(msg_final_img2)

            msg_final_img3 = CompressedImage()
            msg_final_img3.header.stamp = rospy.Time.now()
            msg_final_img3.format = "jpeg"
            msg_final_img3.data = np.array(cv2.imencode('.jpg', dst)[1]).tostring()
            self._pub5.publish(msg_final_img3)

        # publishing calbrated and Bird's eye view as raw image
        elif self.selecting_pub_image == "raw":
            # self._pub2.publish(self.cvBridge.cv2_to_imgmsg(img_output, "bgr8"))
            # self._pub3.publish(self.cvBridge.cv2_to_imgmsg(img_output2, "bgr8"))
            self._pub6.publish(self.cvBridge.cv2_to_imgmsg(dst, "bgr8"))
...
if __name__ == '__main__':
    rospy.init_node('color_redefinition')
    node = ColorRedefinition()
    node.main()
